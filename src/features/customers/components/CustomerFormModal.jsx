...
const CustomerFormModal = ({ isOpen, onClose, initialData, onSubmit, roles, getCountriesApi, getRegionsApi, getCitiesApi }) => {
    ...
    // Encuentra el role_id para 'Cliente'
    const customerRoleId = React.useMemo(() =>
        roles?.find(role => role.name === 'Cliente')?.role_id,
        [roles]
    );
    ...
    useEffect(() => {
        if (isOpen) {
            if (initialData) {
                // Modo edición de cliente
                setIsNewCustomer(false);
                setFormData({
                    user_id: initialData.user_id, // Necesario para updateUser
                    ...
                    city_name_input: initialData.city?.name || '',
                    role_id: initialData.role?.role_id || customerRoleId || '',
    ...
    }, [initialData, isOpen, customerRoleId]);
    ...
    const handleChange = (e) => {
        handleChangeHelper(e, formData, setFormData, isNewCustomer);
    };
    ...
    const handleSubmit = (e) => {
        ...
        if (!customerRoleId) {
            Swal.fire({
                ...
                text: 'El rol "Cliente" no se encontró en la lista de roles proporcionados. Asegúrate de que el rol exista y se cargue correctamente.',
            });
            return;
        }
        ...
        if (isNewCustomer && !formData.password) {
            Swal.fire({
                icon: 'error',
                title: 'Contraseña requerida',
                text: 'Para nuevos clientes, la contraseña es obligatoria.',
            });
            return;
        }
        ...
        // Aseguramos que el role_id sea el del cliente
        dataToSubmit.role_id = customerRoleId;
                    ...
                    <div className="flex min-h-full items-center justify-center p-4 text-center">
                        ...
                        >
                            <Dialog.Panel className="w-full max-w-4xl transform overflow-visible rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                                ...
                                >
                                    {isNewCustomer ? 'Añadir Nuevo Cliente' : 'Editar Cliente'}
                                    <button
                                        type="button"
                                ...
                                <div className="mt-4">
                                    ...
                                    <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                        {/* Los inputs ya están dentro de sus propios <div>, por lo que se ajustarán automáticamente */}
                                        <div>
                                        ...
                                        <div>
                                            ...
                                            <input type="number" name="phone" id="phone" value={formData.phone} onChange={handleChange} autoComplete='off' className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
                                        </div>

                                        <div className="col-span-1 lg:col-span-3"> {/* Cambio: col-span-1 para móviles, lg:col-span-3 para escritorio */}
                                        ...
                                        {/* Se ajusta el col-span para los campos de País, Región y Ciudad */}
